    ...
    async createViaLNURL(signer, lnurl, amountData, lps, options, additionalParams, abortSignal) {
        ...
        try {
            ...
            if (amountData.exactIn) {
                const { invoice: dummyInvoice } = await LNURL_1.LNURL.useLNURLPay(payRequest, BigInt(payRequest.minSendable) / 1000n, options?.comment, this.options.getRequestTimeout, _abortController.signal);
                return lps.map(lp => {
                    return {
                        quote: this.getIntermediaryQuoteExactIn(signer, amountData, payRequest, lp, dummyInvoice, _options, {
